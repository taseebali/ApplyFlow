...
- **Dashboard** — every application is saved locally in full: the job description (even from ATS flows where it lives on a different page than the form), the match score and keyword gap, the bank variant ids of the exact bullets that went out, the figures the model estimated, and both the resume and cover letter as the .docx files actually sent. The dashboard that displays all of it is a page inside the extension, opened from the icon in the panel header — no host, no server, no account, nothing deployed. It reads the same list three ways: a **table** to sort and scan, a **board** where dragging a card between columns sets its status, and a **gallery** ordered by how well each posting matched. Search reaches the job description itself, so "who wanted Kubernetes" is a question it can answer. Each application carries properties only you can know — status, priority, tags, a next action with a date, and what the posting said about salary — edited in place and saved as you type; the extension validates every one of them again on arrival and accepts writes to those six fields and nothing else. The panel shows a summary and exports CSV, but doesn't set properties. Because it is an extension page rather than a website, no outside page can reach it: the extension answers no external origin at all, in any build.
