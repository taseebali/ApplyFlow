...
A browser extension that fills job application forms from your own locally-stored profile, attaches the right resume/cover letter, and keeps a record of every application in a dashboard — all without leaving the tab you're applying from.
...
- **One instance per tab** — every application keeps its own fill summary, attachments, and drafts. Start drafting on one posting, switch to another tab and fill it, and come back to find the first one's answers waiting. Drafting runs in the background, so it keeps going with the panel closed.
...
- **Dashboard** — every application is saved locally in full: the job description (even from ATS flows where it lives on a different page than the form), the match score and keyword gap, the bank variant ids of the exact bullets that went out, the figures the model estimated, and both the resume and cover letter as the .docx files actually sent. The extension includes a dashboard that displays all of it — a static page with no server, no database, and no account, it holds nothing itself; it asks the extension for records over the browser's own messaging API and shows nothing if the extension isn't installed. It reads the same list three ways: a **table** to sort and scan, a **board** where dragging a card between columns sets its status, and a **gallery** ordered by how well each posting matched. Search reaches the job description itself, so "who wanted Kubernetes" is a question it can answer. Each application carries properties only you can know — status, priority, tags, a next action with a date, and what the posting said about salary — edited in place and saved as you type; the extension validates every one of them again on arrival and accepts writes to those six fields and nothing else. The panel shows a summary and exports CSV, but doesn't set properties. The dashboard is not deployed: it runs locally (`cd dashboard && npm run dev`, port 5174), and only a development build of the extension will answer it. A release build trusts no external page at all — the host to trust has to exist and be owned before it can be listed, so deploying it means registering a real host, then adding that origin to `externally_connectable` in `extension/wxt.config.ts` and to `ALLOWED_ORIGINS` in `extension/lib/dashboard-bridge.ts`.
